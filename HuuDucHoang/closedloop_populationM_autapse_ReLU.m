function z = closedloop_populationM_autapse_ReLU(t,u,params)
...
% State vector u is of length 4*N + 4
%   u = [ v(1:N);
%         n(1:N);
%         h(1:N);
%         s(1:N);
...
x = -E1*(vollung-Vol0) + E2*alpha;

% read softplus steepness from params
if isfield(params,'softplus_r') && ~isempty(params.softplus_r)
    rr = params.softplus_r;
    dvolrhs = softplus(x, rr);
else
    dvolrhs = max(0, x);
end
